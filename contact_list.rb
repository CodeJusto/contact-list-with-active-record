...
def initialize
  ...
  # Add new contacts
    ...
    puts "ID: #{new_contact.id} | Name: #{new_contact.name} | Email: #{new_contact.email} | Phone: #{new_contact.full_number}"
  # List all contacts
  elsif ARGV[0] == "list"
    puts "Listing all contacts..."
    retrieved_array = Contact.all
    retrieved_array.each do |line|
      puts "ID: #{line.id} | Name: #{line.name} | Email: #{line.email}"
    end
  # Show a single contact
    ...
    while true
      begin
        puts "ID: #{retrieved_array.id} | Name: #{retrieved_array.name} | Email: #{retrieved_array.email}"
        break
      rescue
